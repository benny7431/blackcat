...
module.exports = {
  ...
  async execute(message, args) {
    ...
    if (!message.member.permission.has(Permissions.FLAGS.MANAGE_MESSAGES) && message.author.id !== "669194742218752070") return message.channel.send("❌ ┃ 你沒有足夠的權限").catch(console.error);
    if (!message.guild.me.permission.has(Permissions.FLAGS.MANAGE_MESSAGES)) return message.channel.send("❌ ┃ 我沒有相關的權限!").catch(console.error);
