...
module.exports = {
  ...
  execute(message, args) {
    let user = message.options.getUser("用戶") || args[0];
    if (message.mentions.members.size >= 1) {
      if (message.mentions.members.first().id === message.author.id) return message.channel.send("❌ ┃ 請尊重生命，不要自殺...")
        .catch(console.error);
      else if (message.mentions.members.first().id === "848006097197334568") return message.channel.send("❌ ┃ 你不能殺掉我!!!")
        .catch(console.error);
      else if (message.mentions.members.first().id === "669194742218752070") return message.channel.send("❌ ┃ 你不能殺掉開發者!!!")
        .catch(console.error);
    }
